use tauri::Manager;
use sleekly_backend::{init_app_state, start_axum_server, state::AppState};
use uuid::Uuid;
...
pub fn run() {
  tauri::Builder::default()
    .setup(|app| {
      // 1. Resolve OS-specific App Data Directory for Sleekly
      ...
      let db_path = db_dir.join("sleekly.db");
