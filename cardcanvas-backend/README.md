# CardCanvas Backend Library & CLI

High-performance Rust API for CardCanvas, built with Axum and SQLx (SQLite).
...
1.  Copy `.env.example` to `.env` and update your `DATABASE_URL` (e.g. `sqlite://cardcanvas.db`).
